...
## v0.8 (the big-model bet)

The v0.7 distribution push met silence, recorded here without varnish. The response is not louder posting but a bet on the loudest niche there is: running very large models on hardware you own. It is a product bet that doubles as an audience bet - and it lands on exactly the hardware of the only inbound signal the project ever received.
...
- Keep listening: the posts stay up, the GPU issue form stays open, and the big-model story - once it ships and benches honestly - is a genuinely new reason to post again, not a repost.
...
- ds4 (DwarfStar): promoted to v0.8 as the third engine. The original objection (beta, volatile model-support policy) is being re-verified against its current release discipline as the first step of the integration.
