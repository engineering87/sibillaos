...
- Embeddings through the gateway for local RAG: /v1/embeddings behind the same keys, an embedding role in the signed catalog (never a chat candidate: the selector skips it, `model use` refuses it), nomic-embed-text v1.5 as the first entry with recorded digests, and `sibilla model pull` to stage a model without switching what is served (docs/embeddings.md). In progress on the branch.
- Developer kit around `sibilla connect`: `--env` emits the three variables every OpenAI SDK and framework reads (LangChain, LlamaIndex, plain clients), `--mcp` prints the ready `claude mcp add` command with the key filled in, `--snippet python|node` prints SDK boilerplate, and `--write` places the aider configuration next to Continue (which also covers JetBrains). Cline keeps its settings in VS Code internal state, so it stays paste-by-hand, said honestly. A remote-workstation kit (one command that configures the developer machine) is deferred to a future cycle: it deserves design, the key would cross the shell history. In progress on the branch.
