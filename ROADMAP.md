...
- Supply freshness, opening the cycle: a considered engine pin refresh (ollama, and the vLLM container tag) with the whole pipeline as the safety net, plus sha256 digests recorded for every entry of the catalog (tools/update-digests.sh exists for exactly this) and any new model families the refreshed engine enables. "Does it run the new model" is the first question every visitor asks, and recent ROCm support lives in recent engines, so this precedes the GPU work.
- Consumer GPUs, following the only real inbound signal the project has received (a question about Strix Halo/ROCm): AMD detection in llmd-hw-detect, the ROCm variant of the pinned ollama install where it applies, a validation guide, and an explicit call for NVIDIA and AMD testers. CI has no GPU, so this is where external hands matter most; the audience that would adopt SibillaOS owns RTX cards and Ryzen APUs, and a first contact that stumbles there is the end of the funnel.
