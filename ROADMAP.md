...
## v0.6 (agents, the road off the network, configuration as code)

Three audiences this cycle: agent users who want a local model their frameworks can discover, the on-premise deployments where the network is not a given, and whoever runs more than one machine.

- MCP server, promoted from exploratory: `sibilla mcp enable` exposes the local model to agent frameworks (Claude Code and any Streamable HTTP client) as `chat` and `list_models` tools at /mcp, behind the same gateway keys. A stateless dependency-free server (Python standard library, loopback only, never sees the keys), aligned with where the protocol is going. Done, exercised end to end in CI.
- Air-gapped installs, moved from v0.4 and built in bricks. `sibilla model import FILE` brings a model in from a file instead of the network, verified against the per-quant sha256 digests of the signed catalog before it is accepted (the same fail-closed policy as `model use`), so a USB stick becomes a legitimate, reviewable delivery channel. On top of it: tools/build-airgap-payload.sh prepares the companion volume on a connected machine (files resolved on Hugging Face by their catalog digest and verified before entering the payload, a declared profile rides along), and llmd-firstboot detects the SIBILLA-AIRGAP volume, imports through the digest gate and serves without ever reaching for the network. CI deploys the cloud image in a network-restricted VM with only the payload attached and requires a real completion (docs/airgap.md). vLLM payloads are out of scope for now. Done.
- Configuration as code: a declared profile (one KEY=value file: model, TLS, metrics, MCP, WebUI) and an idempotent `sibilla apply` that converges the machine onto it, with `apply export` turning a configured machine into a profile. Keys stay out by design. cloud-init or a fleet tool writes /etc/llmd/profile and first boot picks it up, which is the fleet story that was parked in exploratory; on the apt path the same file works through `sibilla setup`. Done, exercised in CI (drift, idempotence, invalid values, no key material in exports).
- Gateway rate limiting, re-scoped after hitting an upstream packaging fact: Caddy's rate-limit handler is a separate plugin requiring a custom binary, and a custom binary would trade away the automatic security updates the appliance gets from the Ubuntu caddy package, which is the worse deal. The committed baseline is kernel-level per-IP limiting against unauthenticated floods, which is proxy-agnostic and survives any future gateway decision: appliance first boot installs a hashlimit rule on new gateway connections (2/s sustained, burst 60 per source, generous for keep-alive clients and a wall for floods), sibilla remove strips it, CI asserts it in the rules file and loaded in the kernel. Done. Per-key fairness limiting is deferred until a real deployment asks for it, and the designated path for it is evaluated and recorded (architecture changelog v0.11): a migration to nginx, whose native limit_req takes any variable as the zone key including the bearer header, and which sits in Ubuntu main; the trade (Caddy's built-in local CA, native ACME and Prometheus metrics would all need replacements we maintain) is judged worth it only against real demand, not preemptively.
- Adoption and reliability extras: `sibilla bench` measures first-token latency and generation speed through the gateway and prints a shareable markdown table (a ten-minute reason to try SibillaOS, and real hardware data back from testers). The upgrade path, a v1.0 criterion that was only a sentence, is now an assert: CI installs the latest published release from the live APT repository, upgrades to the branch packages with the exact command the docs give, and requires the serving machine to survive with its key, model and gateway intact. In progress on the branch.
- Embeddings through the gateway for local RAG: /v1/embeddings behind the same keys, an embedding role in the signed catalog (never a chat candidate: the selector skips it, `model use` refuses it), nomic-embed-text v1.5 as the first entry with recorded digests, and `sibilla model pull` to stage a model without switching what is served (docs/embeddings.md). Done.
- Developer kit around `sibilla connect`: `--env` emits the three variables every OpenAI SDK and framework reads (LangChain, LlamaIndex, plain clients), `--mcp` prints the ready `claude mcp add` command with the key filled in, `--snippet python|node` prints SDK boilerplate, and `--write` places the aider configuration next to Continue (which also covers JetBrains). Cline keeps its settings in VS Code internal state, so it stays paste-by-hand, said honestly. A remote-workstation kit (one command that configures the developer machine) is deferred to a future cycle: it deserves design, the key would cross the shell history. Done.
...
- Open WebUI web interface: packaged and CI-exercised around the container, the login-and-chat flow never validated by hand (folded into v0.6 above; checklist in docs/validation/webui.md).
...
- Engine pin refresh as a recurring discipline: a considered ollama (and vllm) bump with digests re-verified and the whole pipeline as the safety net. High value for model freshness, riskier than a feature; the natural opener for v0.7.
